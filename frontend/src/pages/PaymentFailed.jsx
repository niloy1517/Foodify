    ...
    <div className='w-full flex justify-center '>
      <div className='w-[550px] shadow-2xl py-10 rounded-2xl my-26'>
        <div className='w-full flex flex-col justify-center items-center'>
          <img className='w-20' src={assets.payment_failed} alt="" />
          <h1 className='text-2xl font-semibold text-red-500'>Payment Failed</h1>
          <p className='text-center flex-wrap pt-4 text-[17px] px-10'>We couldn't process your payment. Please check your payment details and try again.</p>
        </div>
        ...
        <div className='w-full flex flex-col gap-3 text-[18px] font-semibold text-gray-700 p-10'>
          ...
          <p className='flex justify-between'>Payment Method <span>{order?.gatewayResponse?.card_issuer}</span></p>
          <p className='flex justify-between'>Date & Time <span>{date_TimeFormat}</span></p>
          <p className='flex justify-between'>Reason <span>{order?.gatewayResponse?.error}</span></p>
          <br />
          <p className='flex justify-between'>Attempted Amount <span>Tk {order?.gatewayResponse?.amount}</span></p>
        </div>
