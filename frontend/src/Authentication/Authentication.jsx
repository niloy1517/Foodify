...
const Authentication = () => {
    const { setShowAuthenticationPopup, loginPopup, setLoginPopup, isLoginPage, setIsLoginPage } = useContext(storeContext)
    ...
    const handleRegister = async () => {
        try {
            const response = await axiosInstance.post('/user/registration', registerData)
    ...
    const handleSendRestPasswordOtp = async () => {
        try {
            const response = await axiosInstance.post('/user/reset/password/otp', { email })
    ...
    return (
        <div onClick={() => setShowAuthenticationPopup(false)} className='fixed inset-0 z-50 bg-black/20 flex justify-center items-center'>
            <div className=' z-20'>
                {
                    loginPopup &&
                    <div onClick={(e) => e.stopPropagation()} className='bg-white text-black w-80 md:w-100 h-100 flex flex-col px-6 py-6 rounded-[8px]'>
